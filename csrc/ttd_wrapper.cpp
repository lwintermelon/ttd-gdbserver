...
/* ---- Helper: DataAccessType -> DataAccessMask ----
 * The SDK reports a watchpoint hit as a DataAccessType *enum* (Read=0,
 * Write=1, Execute=2, CodeFetch=3, ...), but the C ABI and the Rust matcher
 * use a DataAccessMask *bitmask* (Read=1<<0, Write=1<<1, Execute=1<<2, ...).
 * Convert at the boundary so Rust never has to know the enum values. */
static uint8_t access_type_to_mask(TTD::Replay::DataAccessType t) {
    /* Use the SDK's own conversion so the enum-value -> bitmask mapping can
     * never drift if the SDK adds a new DataAccessType. */
    return static_cast<uint8_t>(ConvertDataAccessTypeToMask(t));
}
...
static void fill_result(TtdReplayResult* out, ICursorView::ReplayResult const& rr) {
    ...
    if (rr.StopReason == EventType::MemoryWatchpoint) {
        ...
        out->wp_access_type = access_type_to_mask(rr.MemoryWatchpoint.AccessType);
    } else {
        out->wp_address     = 0;
        out->wp_size        = 0;
...
static bool __fastcall WatchpointCbShim(
    ...
    if (cur->wp_cb) {
        uint32_t tid = 0;
        if (thread) {
            tid = static_cast<uint32_t>(thread->GetThreadInfo().Id);
        }
        return cur->wp_cb(
            cur->wp_ctx,
            static_cast<uint64_t>(result.Address),
            result.Size,
            access_type_to_mask(result.AccessType),
            tid
        );
    }
...
struct IndexProgressCtx {
    TtdIndexProgressCb cb;
    void* ctx;
};

static void __stdcall IndexProgressCbShim(void const* context, TTD::Replay::IndexBuildProgressType const* p) {
    if (!p || p->KeyframeCount == 0) {
        return;
    }
    auto* ic = static_cast<IndexProgressCtx const*>(context);
    if (ic && ic->cb) {
        ic->cb(ic->ctx, p->KeyframesProcessed, p->KeyframeCount);
    } else {
...
static int build_index_impl(TtdEngine* eng, TtdIndexProgressCb cb, void* ctx) {
    if (!eng) return -1;
    IndexProgressCtx ic{ cb, ctx };
    // The progress callback must be non-null: passing nullptr crashes the
    // engine in this SDK version (0.9.5).
    auto status = eng->engine->BuildIndex(IndexProgressCbShim, &ic);
    return (status == TTD::Replay::IndexStatus::IndexFileLoaded) ? 0 : -2;
}

int ttd_engine_build_index(TtdEngine* eng) {
    return build_index_impl(eng, nullptr, nullptr);
}

int ttd_engine_build_index_cb(TtdEngine* eng, TtdIndexProgressCb cb, void* ctx) {
    return build_index_impl(eng, cb, ctx);
}
