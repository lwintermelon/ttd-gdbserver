...
/* Callback types */
/* The watchpoint callback receives the OS thread id of the thread that hit
 * the watchpoint, so the Rust side can apply per-thread breakpoint filters.
 * Access type is a DataAccessMask bitmask (see access_type_to_mask). */
typedef bool (*TtdWatchpointCb)(void* ctx, uint64_t addr, uint64_t size, uint8_t access_type, uint32_t thread_id);
typedef void (*TtdProgressCb)(void* ctx, TtdPosition pos);
/* Index-build progress: (ctx, keyframes_processed, keyframes_total).
 * Only invoked while ttd_engine_build_index_cb is on the stack. */
typedef void (*TtdIndexProgressCb)(void* ctx, uint32_t processed, uint32_t total);
 ...
 * Reports progress through the built-in stderr reporter.
 * Returns 0 on success, non-zero on failure. */
int ttd_engine_build_index(TtdEngine* eng);
/* Like ttd_engine_build_index, but reports keyframe progress through `cb`.
 * Passing cb == NULL selects the built-in stderr reporter (the SDK crashes
 * on a nullptr progress callback, so an internal shim is always installed). */
