...
GDB 前端构建在目标层 `DebugTarget` trait 之上（`src/target.rs`）：
...
- **执行**：`vCont;c`/`vCont;s`/`vCont;r`（范围单步）、`s`/`c`、
  ...
  范围单步由后端在无观察点的 step cursor 上执行，因此源码级 `next`/
  `stepout` 不会每指令重新定位 query cursor。
...
- **信号 / 模式**：`QPassSignals`/`QProgramSignals`/`vCtrlC` 返回 `OK`；
  ...
  回放没有"spawn"。`QListThreadsInStopReply` 同样不支持：需要给 gdbstub
  fork 打第四个补丁，而每次 stop 只能省一次往返。
...
宿主上是单线程的。这与 rr 对外声明的能力一致。从未出现在 trace 中的
线程 id 没有状态，查询返回 `E`。
...
| `monitor ttd events [n]` | 异常事件摘要（可选显示条数，默认 20） |
| `monitor ttd stats` | 回放计数器：查询数、光标定位次数、单步数、回放次数、物理观察点添加/删除次数 |
...
| 测试 | 覆盖 | 需要 TTD？ |
|---|---|---|
| `cargo test --lib` | 单元测试：断点管理、停止原因映射、TargetRunner 并发核心、报文解析、qXfer/XML、checkpoint、`monitor ttd` 渲染 | 否 |
| `cargo test --test gdb` | GDB RSP 握手/可逆命令、mock 后端、GDB RSP 协议面、跨会话持久性 | 否 |
| `cargo test --test target` | TtdProcess 目标层 | 是（无 trace 时计为 ignored） |
| `cargo test --test gdb_ttd` | GDB RSP + 真实 trace | 是（无 trace 时计为 ignored） |
| `cargo test --test ttd` | TTD FFI 层，含安全回调封装 | 是（无 trace 时计为 ignored） |

```powershell
# 无 TTD trace（需要 trace 的测试如实报告为 ignored）
