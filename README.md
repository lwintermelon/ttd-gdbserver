...
```bash
dlv ttd <trace.run> --exe <recorded executable>
```
...
The GDB frontend is built on the `DebugTarget` trait (`src/target/mod.rs`):
...
- **Execution**: `vCont;c`/`vCont;s`/`vCont;r` (range step), `s`/`c`,
  `bc`/`bs` (reverse), `vRun` (restart at a position), `D` (detach).
...
single-threaded on the host. This matches what rr advertises.
...
| `monitor ttd events [n]` | exception-event summary (full list via `qTTDEvents` later) |
...
| Test | Count | Needs TTD? |
|---|---|---|
| `cargo test --lib` (breakpoint mgmt, stop-reason mapping, XML escaping) | 22 | No |
| `cargo test --test gdb` (GDB RSP handshake/reverse, mock backend, GSP surface) | 24 | No |
| `cargo test --test target` (TtdProcess target layer) | 15 | Yes (auto-skip without trace) |
| `cargo test --test gdb_ttd` (GDB RSP + real trace) | 4 | Yes (auto-skip without trace) |
| `cargo test --test ttd` (TTD FFI layer) | 12 | Yes (required) |

```powershell
# Without a TTD trace (46)
cargo test --lib --test gdb

# With a TTD trace (all 77)
...
$env:DELVE_TTD_SERVER = "<this project>\target\debug\ttd-gdbserver.exe"
$env:TERM = "dumb"
dlv.exe ttd <trace.run> --exe <recorded executable> --init <script>
```
