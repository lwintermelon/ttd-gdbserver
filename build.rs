...
fn main() {
    ...
    // `has_ttd_trace` gates the trace-dependent integration tests: with no
    // TTD_TRACE_PATH the suites' #[cfg_attr(not(has_ttd_trace), ignore)]
    // tags expand to #[ignore], so libtest reports them as skipped.
    println!("cargo:rerun-if-env-changed=TTD_TRACE_PATH");
    // Declare the custom cfg name so rustc type-checks every use of it
    // (an unexpected_cfgs warning would fire if it were misspelled).
    println!("cargo::rustc-check-cfg=cfg(has_ttd_trace)");
    if env::var_os("TTD_TRACE_PATH").is_some() {
        println!("cargo:rustc-cfg=has_ttd_trace");
    }
    // Rebuild the C++ shim when any of its sources or headers change. Without
    // these directives cargo only rebuilds when build.rs itself changes, so
    // editing ttd_wrapper.cpp silently kept the stale object.
    println!("cargo:rerun-if-changed=csrc/ttd_wrapper.cpp");
    println!("cargo:rerun-if-changed=csrc/ttd_wrapper.h");
    println!("cargo:rerun-if-changed=csrc/ttd_bindings.h");
